...
async function loadPayments() {
  ...
  const total = await fetchJson(`${API}/total_spent/${requireUser()}`);
    paymentSummary.innerHTML = `<div class='card'><div class='card-title'>Payment Preferences</div>
      <div>Cash: ${myPayment.Cash? '✅':'❌'} | Online: ${myPayment.Online? '✅':'❌'}</div>
...
btnNewSession && (btnNewSession.onclick = async ()=>{
  // simple prompt-based create
  const veh = prompt('Vehicle ID (e.g. 1)'); if (!veh) return;
  ...
  const payload = { User_ID: requireUser(), Veh_ID: Number(veh), StartTime: start, EndTime: end, Energy_Consumed: Number(energy), Charger_ID: Number(charger) };
  ...
  const j = await res.json(); if (j.ok) { alert('Session created'); loadDashboard(); }
});
